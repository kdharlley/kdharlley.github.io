...
class Uno():
    ...
    def _drawHand(self):
        ...
        """
        drawn_cards=[]
        while len(drawn_cards) < Uno.STARTING_HAND_SIZE and len(self.deck)!=0:
    ...
    def play(self):
        ...
        while not self._isGameOver():
            next_player_index= self.gameState.nextPlayer%len(self.players)
            next_player= self.players[next_player_index]
            cards_to_draw = self.gameState.numExtraCardDraw
            self.gameState.numExtraCardDraw= 0
            while cards_to_draw!=0 and len(self.deck)!=0:
                next_player.hand.append(self.deck.pop(0))
                cards_to_draw+=-1
            self._performPlayerTurn(next_player)
